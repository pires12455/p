	...
	// Filter Banned People
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'Aqui está o seu adesivo ')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[WAIT] Em andamento⏳ aguarde um pouco ae man!', id)
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, 'Isto é obrigado!', Id)
                    })
                    .catch(() => {
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, 'Prós como ... ed)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Ini kak..', id)
                } catch {
                    aruga.reply('Bom, o processo falhou, irmão, já está correto? .. ', id)
                }
            } else {
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				await aruga.sendFileFromUrl(from, linkhd, '', 'Aqui está o vídeo ', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'Aqui está o vídeo ', id)
					.catch(() => {
        ...
        case 'chord':
            ...
            .catch(() => {
                aruga.reply(from, 'Há um erro! ', Eud)
            })
            break
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		aruga.reply(from, 'Desculpe, não sei o que é este anime, certifique-se de que as imagens a serem pesquisadas não estão desfocadas / cortadas', id)
                	}
        ...
        case 'resi':
            ...
            const kurirs = ['jne ',' pos ',' tiki ',' wahana ',' jnt ',' rpx ',' sap ',' sicepat ',' pcp ',' jet ',' dse ',' primeiro ',' ninja ' , 'leão', 'idl', 'rex']
            ...
            console.log('Verificar recibo não ', args[1], 'por expedição ', args[0])
	    ...
	    case 'add':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo! ', En)
        ...
        case 'kick':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo! ', En)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupo! ', Id)
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \ NPPor favor, marque uma ou mais pessoas a serem excluídas ', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está errado.\nIncapaz de emitir sua própria conta de bot ', id)
            await aruga.sendTextWithMentions(from, `Request diterima, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await aruga.sendText(from, 'Falha, você não pode desconectar o administrador do grupo. ')
        ...
        case 'promote':
            ...
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \nIncapaz de promover sua própria conta de bot ', id)
        ...
        case 'demote':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo! ', En)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupo! ', Id)
            if (mentionedJidList.length !== 1) return aruga.reply(from, 'Desculpe, só pode remeter 1 usuário ', id)
            ...
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \ N Não é possível demonstrar a própria conta do bot ', id)
        ...
        case 'del':
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo! ', En)
            if (!quotedMsg) return aruga.reply(from, `Desculpe, o formato da mensagem está errado, por favor. \ nReplicar mensagem ao bot com uma legenda ${prefix}del`, id)
		...
		case 'simi':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos! ', Id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo! ', En)
			if (args.length !== 1) return aruga.reply(from, `Para habilitar simi-simi no Chat de Grupo\n\nUsar\n${prefix}simi on --ativar\n${prefix}simi off --desligue\n`, id)
			if (args[0] == 'on') {
				simi.push(chatId)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
                aruga.reply(from, 'Ative o bot simi-simi! ', En)
			} else if (args[0] == 'off') {
				let inxx = simi.indexOf(chatId)
				simi.splice(inxx, 1)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
				aruga.reply(from, 'Desative o bot simi-simi! ', Id)
			} else {
				aruga.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			}
			break
		...
		case 'kasar':
			...
			if (args[0] == 'on') {
				...
				aruga.reply(from, 'O recurso Anti-Violent foi ativado ', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				aruga.reply(from, 'O recurso anti-violento foi desativado ', id)
			} else {
				aruga.reply(de, `Para ativar o recurso Palavras fortes no bate-papo em grupo\n\nPara que serve esse recurso? Se alguém disser palavras duras, ele receberá uma multa\n\nUsar\n${prefix}áspero ligar - ligar\n${prefix}kasar off --desligue\n\n${prefix}reset --redefinir o valor da multa`, id)
			}
			break
		case 'reset':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos! ', Id)
		...
		case 'mutegrup':
            ...
            if (args[0] == 'on') {
				...
				aruga.setGroupToAdminsOnly(groupId, false).then(() => aruga.sendText(from, 'Alterado com sucesso para permitir que todos os membros conversem! '))
			} else {
				aruga.reply(from, `Para alterar as configurações de bate-papo em grupo para que apenas o administrador possa bater papo\n\nUsar:\n${prefix}mutegrup on --ativar\n${prefix}mutegrup off --desligue`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        ...
        if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupo! ', Id)
            ...
            aruga.reply(from, 'Sucesso chutar todos os membros ', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários de bot!', id)
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários de bots ', id)
        ...
        case 'clearall': //menghapus seluruh pesan diakun bot
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para o proprietário do bot, id)
            const allChatx = await aruga.getAllChats()
            for (let dchat of allChatx) {
                await aruga.deleteChat(dchat.id)
            }
            aruga.reply(from, 'Sucesso, limpar todo o bate-papo! ', id)
            break
        default:
            break
        }
		...
		if(!isCmd && isGroupMsg && isNgegas) {
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        const denda = db.get('group').filter({id: groupId}).map('members['+isIn+']').find({ id: pengirim }).update('multar', n => n + 5000).write()
                        if(denda){
                            await aruga.reply(from, "Não seja um palavrão estúpido\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
